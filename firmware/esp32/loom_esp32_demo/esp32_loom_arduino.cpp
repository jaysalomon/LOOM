...
void Esp32Loom::read_sensors() {
    // Simulated sensors for Arduino demo
    topology->hormones.light_level = (float)random(30, 90) / 100.0f;
    topology->hormones.temperature = (float)random(200, 350) / 10.0f;  // 20-35C
    topology->hormones.touch_sensitivity = (float)random(0, 100) / 100.0f;
}

void Esp32Loom::sensor_input(const char* sensor_name, float value) {
    // Update hormones based on sensor
    if (strcmp(sensor_name, "light") == 0) topology->hormones.light_level = value;
    else if (strcmp(sensor_name, "temperature") == 0) topology->hormones.temperature = value * 40.0f;
    else if (strcmp(sensor_name, "motion") == 0) topology->nodes[0].components[VEC_ACTIVATION] = value;
    else if (strcmp(sensor_name, "touch") == 0) topology->hormones.touch_sensitivity = value;

    // Record experience (simplified)
    uint8_t activated[4] = {0, 1, 2, 3};
    uint8_t sensory[2] = {(uint8_t)(value * 255), 0};
    record_experience(activated, 4, sensory, (int8_t)(value * 127));
}
...
void Esp32Loom::enable_oled_visualization() {
    if (topology->oled_enabled) return;
    ...
    if (!topology->oled_display->begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDRESS)) {
        ...
        delete topology->oled_display;
        topology->oled_display = NULL;
        return;
    }
    ...
    topology->oled_display->println("LOOM Visualization");
...
void Esp32Loom::visualize_topology() {
    ...
    Adafruit_SSD1306* display = topology->oled_display;
    display->clearDisplay();

    // Draw simple network visualization
    for (uint8_t i = 0; i < min((uint8_t)20, topology->num_nodes); i++) {
        int x = (i % 10) * 12 + 6;
        int y = (i / 10) * 24 + 16;
        int radius = (int)(2 + topology->nodes[i].components[VEC_ACTIVATION] * 4);
        display->drawCircle(x, y, radius, SSD1306_WHITE);
    }

    // Stats
    display->setCursor(0, 54);
    display->printf("N:%d E:%d H:%d EMG:%.2f", topology->num_nodes, topology->num_edges,
                    topology->num_hyperedges, topology->emergence_metric);
    display->display();
}

void Esp32Loom::display_stats() {
    Serial.printf("Cycles: %d, Nodes: %d, Edges: %d, Hyperedges: %d, Emergence: %.3f\n",
                  topology->cycles_executed, topology->num_nodes, topology->num_edges,
                  topology->num_hyperedges, topology->emergence_metric);
}
...
void Esp32Loom::print_topology_stats() {
    Serial.printf("Nodes: %d, Edges: %d, Hyperedges: %d\n",
...
float Esp32Loom::cosine_similarity(const ArduinoNodeVector* a, const ArduinoNodeVector* b) {
    ...
    for (uint8_t i = 0; i < ARDUINO_NODE_DIMENSIONS; i++) {
        ...
        mag_a += a->components[i] * a->components[i];
        mag_b += b->components[i] * b->components[i];
    }
...
void Esp32Loom::dump_node(uint8_t node_id) {
    ...
    for (uint8_t i = 0; i < ARDUINO_NODE_DIMENSIONS; i++) {
        Serial.printf("%.3f ", topology->nodes[node_id].components[i]);
    }
    Serial.println();
}
...
void Esp32Loom::initialize_vector(ArduinoNodeVector* vector, uint16_t hash) {
    for (uint8_t i = 0; i < ARDUINO_NODE_DIMENSIONS; i++) {
        vector->components[i] = ((float)((hash >> (i % 8)) & 0xFF) / 255.0f) * 0.2f;
    }

    // Identity baseline
    for (uint8_t i = VEC_IDENTITY; i < VEC_IDENTITY + 4; i++) {
        vector->components[i] = 0.5f;
    }

    // Semantic baseline
    for (uint8_t i = VEC_SEMANTIC; i < VEC_SEMANTIC + 8; i++) {
        vector->components[i] += 0.1f;
    }

    // Activation baseline
    vector->components[VEC_ACTIVATION] = 0.1f;
    vector->components[VEC_ACTIVATION + 1] = 0.0f; // Previous activation

    // Emotional baseline
    for (uint8_t i = VEC_EMOTIONAL; i < VEC_EMOTIONAL + 3; i++) {
...
void Esp32Loom::normalize_vector(ArduinoNodeVector* vector) {
    ...
    if (mag > 0.0f) {
        for (uint8_t i = 0; i < ARDUINO_NODE_DIMENSIONS; i++) {
            vector->components[i] /= mag;
        }
    }
}

void Esp32Loom::hebbian_update_pair(uint8_t node_a, uint8_t node_b, float rate) {
    // Simplified Hebbian adjustment: nudge semantic components closer
    ArduinoNodeVector* a = &topology->nodes[node_a];
    ArduinoNodeVector* b = &topology->nodes[node_b];

    for (uint8_t i = VEC_SEMANTIC; i < VEC_SEMANTIC + 8; i++) {
        float delta = (b->components[i] - a->components[i]) * rate;
        a->components[i] += delta;
        b->components[i] -= delta * 0.5f;  // Asymmetry
    }
}

void Esp32Loom::compute_activation_dynamics() {
    // Decay previous activation
    for (uint8_t i = 0; i < topology->num_nodes; i++) {
        float prev = topology->nodes[i].components[VEC_ACTIVATION];
        topology->nodes[i].components[VEC_ACTIVATION + 1] = prev;
        topology->nodes[i].components[VEC_ACTIVATION] *= 0.95f;
    }

    // Spread activation
    for (uint16_t e = 0; e < topology->num_edges; e++) {
        ArduinoEdge* edge = &topology->edges[e];
        uint8_t target = edge->target;
        float weight = (float)edge->weight / 127.0f;
        topology->nodes[target].components[VEC_ACTIVATION] += weight * 0.05f;
    }

    // Normalize activations
    for (uint8_t i = 0; i < topology->num_nodes; i++) {
        topology->nodes[i].components[VEC_ACTIVATION] = constrain(topology->nodes[i].components[VEC_ACTIVATION], 0.0f, 1.0f);
    }
}

void Esp32Loom::apply_trajectory_evolution() {
    // Placeholder for trajectory evolution logic
}
